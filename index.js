...
async function setHistory (coreId, sessionId, history) {
  ...
  if (redis) {
    ...
    if (DEBUG) console.log('[REDIS:set]', key, 'len=', trimmed.length)
  } else {
    RAM.set(key, trimmed)
    if (DEBUG) console.log('[RAM:set]', key, 'len=', trimmed.length)
  }
}
...
app.post('/v1/complete', async (req, reply) => {
  try {
    ...
    // 3) compose messages (Anthropic format requires content blocks)
    const anthroHistory = history.map(m => ({
      role: m.role,
      content: [{ type: 'text', text: m.content }]
    }))

    const userBlock = { role: 'user', content: [{ type: 'text', text: String(prompt) }] }
    const messages = [...anthroHistory, userBlock].slice(-HISTORY_MAX)
    ...
    if (!r.ok) {
      ...
      if (DEBUG) console.error('[ANTHROPIC:error]', r.status, detail)
    ...
    // 5) persist history (our simple role+text schema)
    const newHistory = [
      ...history,
